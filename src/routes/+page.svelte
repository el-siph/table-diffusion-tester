<script lang="ts">
	import PlayerDetails from '$lib/PlayerDetails.svelte';
	import TableDetails from '$lib/TableDetails.svelte';
	import { BroadcastTypes, type ClientState, type ServerResponse } from '$lib/interfaces';
	import VisualDeck from '$lib/VisualDeck.svelte';
	...
	function handleMessage(response: ServerResponse) {
		...
		if (data) {
			const { responseType, payload } = typeof data === 'string' ? JSON.parse(data) : data;

			console.log(`type: ${responseType}, payload: ${JSON.stringify(payload)}`);
			...
			switch (responseType) {
				...
				case BroadcastTypes.confirmation:
					messages = [...messages, `${new Date().toISOString()} - ${payload['message']}`];
						...
						if (key === '_cardDeck') {
							...
							Object.keys(playersData).map((playerId) => {
								if (playerId === state._playerId) {
									...
									state.playerDeck = playerData._cardDeck._cards;
								}
							});
...
<main>
	<div class="action-buttons" class:faded={state.connectedToServer}>
		<button on:click={connectToServer} disabled={state.connectedToServer}
			>{state.connectedToServer ? 'Connected' : 'Connect to Server'}</button
		>

		<input type="text" bind:value={hostValue} placeholder="hostname" />
		<input type="text" bind:value={portValue} placeholder="port" />
	</div>
	<div class="action-buttons" class:faded={!state.connectedToServer || state._tableId}>
		<button
			on:click={handleJoinTable}
			disabled={!state.connectedToServer ||
				state._tableId !== null ||
				tableCodeValue.length < 1 ||
				playerNameValue.length < 1}>{state._tableId ? 'Joined' : 'Join Table'}</button
		>
		<input
			type="text"
	...
	<div class="action-buttons" class:faded={!state._tableId}>
		<button
			...
			disabled={state._playerId === null || state.tableDeck.length > 0}>Generate Table Deck</button
		>

		<button
			on:click={handleShuffleDeck}
			disabled={!state.connectedToServer || !state.tableDeck.length}>Shuffle Table Deck</button
		>

		<button
			on:click={handleDivideDeck}
			disabled={!state.connectedToServer || !state.tableDeck.length}>Divide Table Deck</button
		>

		<button on:click={handleGetTableState} disabled={!state.connectedToServer || !state._tableId}
	...
	<div class="flex flex-row justify-start gap-2">
		<input name="is-verbose-check" type="checkbox" bind:checked={isVerboseMode} />
	...
	<div class="detail-blocks">
		<TableDetails
			...
			{playerList}
			{isVerboseMode}
			on:popDeckToPlayer={(e) => handlePopTableDeckToPlayer(e.detail.popCount)}
			on:popDeckToPile={(e) => handlePopTableDeckToPile(e.detail.popCount)}
		/>
		<PlayerDetails
			{state}
			{isVerboseMode}
