<script lang="ts">
  ...
  import {
    requestDivideDeck,
    requestGenerateTableDeck,
    requestGetTableState,
    requestJoinTable,
    requestPopPlayerToPile,
    requestPopTableDeckToPile,
    requestPopTableDeckToPlayer,
    requestShuffleDeck,
  } from "$lib/websocket-actions";
  ...
  function handleJoinTable() {
    if (socket) {
      requestJoinTable(socket, playerNameValue, tableCodeValue);
    }
  }

  function handleGenerateTableDeck() {
    if (socket && state._tableId) {
      requestGenerateTableDeck(socket, state._tableId);
    }
  }
  ...
  function handleGetTableState() {
    ...
    if (socket && state._tableId) {
      requestGetTableState(socket, state._tableId);
    }
  }

  function handleShuffleDeck() {
    if (socket && state._tableId) {
      requestShuffleDeck(socket, state._tableId);
    }
  }

  function handleDivideDeck() {
    if (socket && state._tableId) {
      requestDivideDeck(socket, state._tableId);
    }
  }

  function handlePopTableDeckToPlayer(popCount: number) {
    console.log("popping from Table to Player");
    if (socket && state._tableId && state._playerId) {
      requestPopTableDeckToPlayer(
        socket,
        state._tableId,
        state._playerId,
  ...
  function handlePopTableDeckToPile(popCount: number) {
    ...
    if (socket && state._tableId && state._playerId) {
      requestPopTableDeckToPile(socket, state._tableId, popCount);
    }
  }

  function handlePopPlayerToPile(popCount: number) {
    console.log("popping from Player to Pile");
    if (state._tableId && state._playerId) {
      requestPopPlayerToPile(socket, state._tableId, state._playerId, popCount);
    }
  }
</script>
